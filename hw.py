...
def infer(path, img_bytes, tries=8):
    ...
    for _ in range(tries):
        if mime and mime.startswith("image/"):
            ...
            r = requests.post(API, headers=headers, data=img_bytes, timeout=60)
        else:
        ...
        if r.status_code == 401:
            raise RuntimeError("Your Hugging Face API key is invalid or expired. Please renew it at https://huggingface.co/settings/tokens")
        if r.status_code == 403:
            raise RuntimeError("Access forbidden. Check your token permissions (it should have 'read' access).")
        if r.status_code == 503:
            print("⏳ Model warming up... retrying in 2s")
            time.sleep(2)
            continue
